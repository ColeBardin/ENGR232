...
% Last Modified: April 27, 2019 by Dr. Robin Carr to correct addpath command re: spurious/Editor folder
% Previous modification: Dec. 30, 2018 by Dr. Robin Carr.
% All 'call' options in UI lines changed to 'CallBack'. 'number' changed to
% 'NumberTitle' in figure command. 
...
if strcmp(action,'initialize')
   ...
   p = path;
   if contains(p, strcat(tempdir, 'Editor'))  % lines 67-69 added by Dr. Carr to fix 'Editor' subfolder issue
       addpath(tempdir) 
   end
   tmpdir = tempdir;
   ll = length(tmpdir);
...
elseif strcmp(action,'delete')
   ...
   trjh = dud.solhand;
   notice = dud.notice;
   if ~isempty(notice)   % Display window
      nstr = get(notice,'string');
      nstr(1:4) = nstr(2:5);
...
elseif strcmp(action,'plotxyfig')  % Build the graph figure.
  ...
  dud = get(ppdisp,'user');
  % Dr. Carr changed line 5098 to work. Dec30/2018 Simple change: 'number' --> 'NumberTitle'
  ppxy = figure('NumberTitle','off',...
		'tag','pplane8','visible','off');
  name = ['pplane8 t-plot # ',int2str(ppxy.Number)];  % Dr. Carr changed ppxy here to ppxy.Number Dec30/2018
  ud.color = dud.color;
  ud.type = 0;
