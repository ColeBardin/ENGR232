...
% Last Modified: April 3, 2019 by Dr. Robin Carr to correct addpath command re: spurious/Editor folder

% Previous Modification: Oct 31, 2017 by Dr. Robin Carr to work with MATLAB 2017B
% All 'call' options in UI lines changed to 'CallBack'
warning ('off','all'); % Turn off all warnings added by Robin Carr
...
%                 John C. Polking Rice University
  ...
  if strcmp(action,'initialize')
    ...
    p = path;
    if contains(p, strcat(tempdir, 'Editor'))  % lines 66-68 added by Dr. Carr to fix 'Editor' subfolder issue 
        addpath(tempdir) 
    end
    tmpdir = tempdir;
    ll = length(tmpdir);
    ...
    butt(2) = uicontrol('style','push',...
			'pos',rwind,...
			'string','Revert',...
			'CallBack','dfield8(''revert'')',...
			'visible','off');
  
    butt(3) = uicontrol('style','push',...
			'pos',pwind,...
			'string','Proceed',...
			'CallBack','dfield8(''proceed'')',...
			'visible','off');
